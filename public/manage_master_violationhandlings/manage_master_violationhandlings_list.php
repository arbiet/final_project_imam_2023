        ...
        <main class="bg-gray-50 flex flex-col flex-1 overflow-y-scroll h-screen flex-shrink-0 sc-hide pb-40">
            <div class="flex items-start justify-start p-6 shadow-md m-4 flex-1 flex-col">
                ...
                <div class="flex flex-row justify-between items-center w-full border-b-2 border-gray-600 mb-2 pb-2">
                    <h1 class="text-3xl text-gray-800 font-semibold w-full">Master Violation Handlings</h1>
                    <div class="flex flex-row justify-end items-center">
                        <a href="<?php echo $baseUrl; ?>public/manage_master_violationhandlings/manage_master_violationhandlings_create.php" class="bg-green-500 hover-bg-green-700 text-white font-bold py-2 px-4 rounded inline-flex items-center">
                    ...
                    <table class="min-w-full">
                        <thead>
                            <tr>
                                ...
                                <th class="text-left py-2">Violation Category</th>
                                <th class="text-left py-2">Score Range</th>
                                <th class="text-left py-2">Follow-Up Action</th>
                                <th class="text-left py-2">Action</th>
                            </tr>
                        </thead>
                        <tbody>
                            <?php
                            // Fetch violation handling data from the database
                            $searchTerm = isset($_GET['search']) ? $_GET['search'] : '';
                            $page = isset($_GET['page']) ? $_GET['page'] : 1;

                            $query = "SELECT * FROM MasterViolationHandlings WHERE ViolationCategory LIKE '%$searchTerm%' LIMIT 15 OFFSET " . ($page - 1) * 15;

                            $result = $conn->query($query);

                            // Count total rows in the table
                            $queryCount = "SELECT COUNT(*) AS count FROM MasterViolationHandlings WHERE ViolationCategory LIKE '%$searchTerm%'";
                            ...
                            ?>
                                <tr>
                                    ...
                                    <td class="py-2"><?php echo $row['ViolationCategory']; ?></td>
                                    <td class="py-2"><?php echo $row['ScoreRangeBottom'] . ' - ' . $row['ScoreRangeTop']; ?></td>
                                    <td class="py-2"><?php echo $row['FollowUpAction']; ?></td>
                                    <td class='py-2'>
                                        <a href="<?php echo $baseUrl; ?>public/manage_master_violationhandlings/manage_master_violationhandlings_detail.php?id=<?php echo $row['HandlingID'] ?>" class='bg-green-500 hover-bg-green-700 text-white font-bold py-2 px-4 rounded inline-flex items-center text-sm'>
                                            <i class='fas fa-eye'></i>
                                        </a>
                                        <a href="<?php echo $baseUrl; ?>public/manage_master_violationhandlings/manage_master_violationhandlings_update.php?id=<?php echo $row['HandlingID'] ?>" class='bg-blue-500 hover-bg-blue-700 text-white font-bold py-2 px-4 rounded inline-flex items-center text-sm'>
                                            <i class='fas fa-edit'></i>
                                        </a>
                                        <a href="#" onclick="confirmDelete(<?php echo $row['HandlingID']; ?>)" class='bg-red-500 hover-bg-red-700 text-white font-bold py-2 px-4 rounded inline-flex items-center text-sm'>
...
<script>
    ...
    function confirmDelete(handlingID) {
        Swal.fire({
            title: 'Are you sure?',
            ...
            if (result.isConfirmed) {
                ...
                window.location.href = `manage_master_violationhandlings_delete.php?id=${handlingID}`;
            }
        });
    }
</script>
