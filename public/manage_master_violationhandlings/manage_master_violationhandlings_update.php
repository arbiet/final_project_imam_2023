...
$handling_id = $violation_category = $score_range_bottom = $score_range_top = $follow_up_action = '';
$errors = array();

// Retrieve the violation handling data to be updated (you might need to pass the handling ID to this page)
if (isset($_GET['id'])) {
    $handling_id = mysqli_real_escape_string($conn, $_GET['id']);

    // Query to fetch the existing violation handling data
    $query = "SELECT * FROM MasterViolationHandlings WHERE HandlingID = ? LIMIT 1";
    $stmt = $conn->prepare($query);
    $stmt->bind_param('s', $handling_id);
    $stmt->execute();
    $result = $stmt->get_result();
    $violation_handling = $result->fetch_assoc();

    // Check if the violation handling exists
    if (!$violation_handling) {
        // Violation handling not found, handle accordingly (e.g., redirect to an error page)
    } else {
        // Populate variables with existing violation handling data
        $violation_category = $violation_handling['ViolationCategory'];
        $score_range_bottom = $violation_handling['ScoreRangeBottom'];
        $score_range_top = $violation_handling['ScoreRangeTop'];
        $follow_up_action = $violation_handling['FollowUpAction'];
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    // Sanitize and validate the input data (similar to create violation handling form)
    ...
    // Update violation handling data in the database
    $query = "UPDATE MasterViolationHandlings 
              SET ViolationCategory = ?, ScoreRangeBottom = ?, ScoreRangeTop = ?, FollowUpAction = ? 
              WHERE HandlingID = ?";
    $stmt = $conn->prepare($query);
    $stmt->bind_param("sssss", $violation_category, $score_range_bottom, $score_range_top, $follow_up_action, $handling_id);

    if ($stmt->execute()) {
        // Violation handling update successful
        // Log the activity for violation handling update
        $activityDescription = "Violation handling updated: $violation_category";
        $currentUserID = $_SESSION['UserID'];
        insertLogActivity($conn, $currentUserID, $activityDescription);
        // Display success message or redirect to violation handling list
        echo '<script>
        Swal.fire({
            icon: "success",
            title: "Success",
            text: "Violation handling update successfully.",
        }).then(function() {
        ...
        // Violation handling update failed
        $errors['db_error'] = "Violation handling update failed.";
        ...
        Swal.fire({
            icon: "error",
            title: "Error",
            text: "Violation handling update failed.",
        });
    </script>';
    }
}
        ...
        <main class="bg-gray-50 flex flex-col flex-1 overflow-y-scroll h-screen flex-shrink-0 sc-hide pb-40">
            <div class="flex items-start justify-start p-6 shadow-md m-4 flex-1 flex-col">
                ...
                <div class="flex flex-row justify-between items-center w-full border-b-2 border-gray-600 mb-2 pb-2">
                    <h1 class="text-3xl text-gray-800 font-semibold w-full">Update Violation Handling</h1>
                ...
                <div class="flex flex-col w-full">
                    ...
                    <div class="flex flex-row justify-between items-center w-full pb-2">
                        <div>
                            ...
                            <p class="text-gray-600 text-sm">Update violation handling information form.</p>
                        </div>
                    </div>
                    <!-- End Navigation -->
                    <!-- Violation Handling Update Form -->
                    <form action="" method="POST" class="flex flex-col w-full space-x-2">
                        ...
                        <button type="submit" class="bg-green-500 hover-bg-green-700 text-white font-bold py-2 px-4 rounded inline-flex items-center mt-4 text-center">
                            ...
                            <span>Update Violation Handling</span>
                        </button>
                    </form>
                    <!-- End Violation Handling Update Form -->
                </div>
                <!-- End Content -->
            </div>
        </main>
