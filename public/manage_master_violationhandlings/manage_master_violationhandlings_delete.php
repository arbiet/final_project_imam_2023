...
// Check if the violation handling ID is provided in the query parameter
...
// Perform the deletion of the violation handling
$query = "DELETE FROM MasterViolationHandlings WHERE HandlingID = ?";
$stmt = $conn->prepare($query);
$stmt->bind_param('i', $id);

if ($stmt->execute()) {
    // Log activity description
    $activityDescription = "Violation handling with HandlingID: $id has been deleted.";
    ...
    // Deletion successful
    $stmt->close();
    $success_message = "Violation handling deleted successfully!";
} else {
    // Deletion failed
    $stmt->close();
    $error_message = "Failed to delete violation handling.";
}
...
if (!empty($success_message)) {
    echo "<script>
        Swal.fire({
            ...
            window.location.href = 'manage_master_violationhandlings_list.php'; // Redirect to the violation handlings list page
        });
        </script>";
