        ...
        <main class="bg-gray-50 flex flex-col flex-1 overflow-y-scroll h-screen flex-shrink-0 sc-hide pb-40">
            <div class="flex items-start justify-start p-6 shadow-md m-4 flex-1 flex-col">
                ...
                <div class="flex flex-row justify-between items-center w-full border-b-2 border-gray-600 mb-2 pb-2">
                    <h1 class="text-3xl text-gray-800 font-semibold w-full">Master Violations</h1>
                    <div class="flex flex-row justify-end items-center">
                        <a href="<?php echo $baseUrl; ?>public/manage_master_violations/manage_master_violations_create.php" class="bg-green-500 hover-bg-green-700 text-white font-bold py-2 px-4 rounded inline-flex items-center">
                ...
                <div class="flex flex-col w-full">
                    ...
                    <table class="min-w-full">
                        ...
                        <tbody>
                            ...
                            $query = "SELECT * FROM MasterViolations WHERE ViolationName LIKE '%$searchTerm%' LIMIT 15 OFFSET " . ($page - 1) * 15;

                            $result = $conn->query($query);

                            // Count total rows in the table
                            $queryCount = "SELECT COUNT(*) AS count FROM MasterViolations WHERE ViolationName LIKE '%$searchTerm%'";
                            ...
                            ?>
                                <tr>
                                    ...
                                    <td class="py-2"><?php echo strlen($row['ViolationType']) > 20 ? substr($row['ViolationType'], 0, 20) . '...' : $row['ViolationType']; ?></td>
                                    <td class="py-2"><?php echo strlen($row['ViolationName']) > 20 ? substr($row['ViolationName'], 0, 20) . '...' : $row['ViolationName']; ?></td>
                                    <td class="py-2"><?php echo $row['Points']; ?></td>
                                    <td class='py-2'>
                                        <a href="<?php echo $baseUrl; ?>public/manage_master_violations/manage_master_violations_detail.php?id=<?php echo $row['ViolationID'] ?>" class='bg-green-500 hover-bg-green-700 text-white font-bold py-2 px-4 rounded inline-flex items-center text-sm'>
                                            <i class='fas fa-eye'></i>
                                        </a>
                                        <a href="<?php echo $baseUrl; ?>public/manage_master_violations/manage_master_violations_update.php?id=<?php echo $row['ViolationID'] ?>" class='bg-blue-500 hover-bg-blue-700 text-white font-bold py-2 px-4 rounded inline-flex items-center text-sm'>
                                            <i class='fas fa-edit'></i>
                                        </a>
                                        <a href="#" onclick="confirmDelete(<?php echo $row['ViolationID']; ?>)" class='bg-red-500 hover-bg-red-700 text-white font-bold py-2 px-4 rounded inline-flex items-center text-sm'>
    ...
    function confirmDelete(violationID) {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                ...
                window.location.href = `manage_master_violations_delete.php?id=${violationID}`;
            }
        });
    }
</script>
